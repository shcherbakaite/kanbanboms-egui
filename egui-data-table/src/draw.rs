...
use crate::{
    viewer::{ColumnHeaderMenuAction, EmptyRowCreateContext, RowViewer, TrivialConfig},
    DataTable, UiAction,
};
...
impl<'a, R, V: RowViewer<R>> Renderer<'a, R, V> {
    pub fn new(table: &'a mut DataTable<R>, viewer: &'a mut V) -> Self {
        ...
        // Restore clipboard preserved when switching between tables (e.g. BOM edit windows)
        if let Some(preserved) = table.preserved_clipboard.take() {
            state.set_clipboard(Some(preserved));
        }
        state = state.tap_mut(|x| {
    ...
    pub fn with_max_scroll_height(mut self, height: f32) -> Self {
        self.config.max_scroll_height = Some(height);
        self
    }
    ...
    pub fn show(mut self, ui: &mut egui::Ui) -> Response {
        ...
        // No POS/ID column: start with data columns only (fork: hide row index column)
        ...
        let max_scroll_h = self.config.max_scroll_height.unwrap_or(f32::MAX);
        builder
                ...
                for (vis_col, &col) in s.vis_cols().iter().enumerate() {
                    ...
                    if resp.dragged() {
                        egui::show_tooltip_text(
                            ctx,
                            ui_layer_id,
                            egui::Id::new("_EGUI_DATATABLE__COLUMN_MOVE__"),
                    ...
                    resp.context_menu(|ui| {
                        ...
                        // Custom column menu extras (e.g. custom fields with checkmarks)
                        let vis_col_indices: Vec<usize> =
                            s.vis_cols().iter().map(|c| c.0).collect();
                        let mut push = |action: ColumnHeaderMenuAction| {
                            match action {
                                ColumnHeaderMenuAction::HideColumn(i) => {
                                    commands.push(Command::CcHideColumn(ColumnIdx(i)));
                                }
                                ColumnHeaderMenuAction::ShowColumn { column, at } => {
                                    commands.push(Command::CcShowColumn {
                                        what: ColumnIdx(column),
                                        at: VisColumnPos(at),
                                    });
                                }
                            }
                        };
                        viewer.column_header_context_menu_extras(
                            ui,
                            s.num_columns(),
                            &vis_col_indices,
                            vis_col.0,
    ...
    fn show_body(
        ...
        let allowed_actions = viewer.allowed_context_menu_actions();
        let action_allowed = |a: UiAction| -> bool {
            allowed_actions
                .as_ref()
                .map_or(true, |set| set.contains(&a))
        };
        ...
        'detect_hotkey: {
            ...
            if !s.is_editing() {
                ctx.input_mut(|i| {
                    i.events.retain(|x| {
                        match x {
                            Event::Copy if action_allowed(UiAction::CopySelection) => {
                                actions.push(UiAction::CopySelection)
                            }
                            Event::Cut if action_allowed(UiAction::CutSelection) => {
                                actions.push(UiAction::CutSelection)
                            }
                            Event::Paste(clipboard) => {
                                ...
                                if i.modifiers.shift {
                                    if action_allowed(UiAction::PasteInsert) {
                                        actions.push(UiAction::PasteInsert);
                                    }
                                } else if action_allowed(UiAction::PasteInPlace) {
        ...
        let _vis_row_digits = s.cc_rows.len().max(1).ilog10();
        let _row_id_digits = table.len().max(1).ilog10();
        ...
        let render_fn = |mut row: egui_extras::TableRow| {
            ...
            // Fork: POS/ID column removed (no row index column)
            ...
            for (vis_col, col) in visible_cols.iter().enumerate() {
                ...
                resp.clone().context_menu(|ui| {
                    ...
                    let system_clip = viewer.get_system_clipboard_for_paste();
                    let can_paste = clip || system_clip.is_some();
                    ...
                    [
                        ...
                        Some((
                            ...
                            "➿",
                            "Clipboard: Paste",
                            if let Some(ref c) = system_clip {
                                UiAction::PasteFromSystemClipboardInPlace(c.clone())
                            } else {
                        ...
                        Some((
                            ...
                            "Clipboard: Insert",
                            if let Some(ref c) = system_clip {
                                UiAction::PasteFromSystemClipboardInsert(c.clone())
                            } else {
                    ...
                    .map(|opt| {
                        ...
                        {
                            if !action_allowed(action.clone()) {
                                return;
                            }
                            if draw_sep {
                                draw_sep = false;
                            ...
                            // Map clipboard variants to base actions for shortcut lookup
                            let lookup_action = match &action {
                                UiAction::PasteFromSystemClipboardInsert(_) => &UiAction::PasteInsert,
                                UiAction::PasteFromSystemClipboardInPlace(_) => &UiAction::PasteInPlace,
                                a => a,
                            };
                            let hotkey = hotkeys
                                .iter()
                                .find_map(|(k, a)| (a == lookup_action).then(|| ctx.format_shortcut(k)));
                    ...
                    // Custom context menu items (viewer-defined); collect to owned to avoid holding viewer borrow
                    let custom_items: Vec<(String, String)> = viewer
                        .custom_context_menu_items(&table.rows[row_id.0])
                        .into_iter()
                        .map(|(l, id)| (l.into_owned(), id))
                        .collect();
                    if !custom_items.is_empty() {
                        if let Some(sink) = viewer.custom_action_sink() {
                            ui.separator();
                            for (label, action_id) in custom_items {
                                if ui.button(&label).clicked() {
                                    *sink = Some((row_id.0, action_id));
                                    ui.close();
                                }
                            }
                        }
        ...
        if let Some(resp) = resp_total.clone() {
            if resp.clicked() | resp.dragged() {
                ...
                // Commit any in-progress edit when user clicks outside the table
                // (e.g. Save button), so uncommitted changes are not lost.
                // Do NOT commit when a popup is open (e.g. ComboBox dropdown) - the user
                // may be clicking on a popup option; committing would close the editor
                // before the selection registers.
                let popup_open = egui::Popup::is_any_open(ctx);
                if s.is_editing() && !popup_open {
                    commands.push(Command::CcCommitEdit);
                }
                if !popup_open {
