...
/// Action for column header context menu extras (e.g. show/hide custom field columns).
#[derive(Debug, Clone)]
pub enum ColumnHeaderMenuAction {
    /// Hide the column at the given index.
    HideColumn(usize),
    /// Show the column at the given index, inserting at the given visible position.
    ShowColumn { column: usize, at: usize },
}
...
pub trait RowViewer<R>: 'static {
    ...
    /// If set, only these actions are shown in the context menu and accepted from shortcuts.
    /// Return `None` to allow all actions (default). Return `Some(set)` to restrict to those actions.
    fn allowed_context_menu_actions(&self) -> Option<std::collections::HashSet<UiAction>> {
        None
    }

    /// Custom context menu items: (label, action_id). Shown after standard items when
    /// `custom_action_sink` returns `Some`. When clicked, the crate sets *sink = Some((row_index, action_id)).
    /// The `row` parameter allows embedding row-specific data (e.g. part id) in action_id to avoid
    /// index mismatch when table data changes between menu open and item click.
    fn custom_context_menu_items(&self, row: &R) -> Vec<(Cow<'_, str>, String)> {
        let _ = row;
        vec![]
    }

    /// If `Some`, the table will show custom menu items from `custom_context_menu_items` and
    /// when one is clicked will set this to `Some((row_index, action_id))`. The viewer/app
    /// should check and take this after the table is drawn.
    fn custom_action_sink(&mut self) -> Option<&mut Option<(usize, String)>> {
        None
    }

    /// Extra items for the column header context menu (e.g. custom field toggles with checkmarks).
    /// Called with: num_columns, visible column indices, insert position for new columns,
    /// and a callback to push hide/show actions. Default implementation does nothing.
    fn column_header_context_menu_extras(
        &mut self,
        _ui: &mut egui::Ui,
        _num_columns: usize,
        _vis_col_indices: &[usize],
        _insert_at: usize,
        _push: &mut dyn FnMut(ColumnHeaderMenuAction),
    ) {
    }

    /// If the viewer can provide system clipboard content for paste (e.g. from arboard),
    /// return it here. Used to show Paste in context menu and to paste when clicked.
...
pub enum UiAction {
    ...
    /// Paste from system clipboard (context menu) - replace selection.
    PasteFromSystemClipboardInPlace(String),
    /// Paste from system clipboard (context menu) - insert new rows.
    PasteFromSystemClipboardInsert(String),
...
pub fn default_hotkeys(context: &UiActionContext) -> Vec<(KeyboardShortcut, UiAction)> {
    ...
    fn shortcut(actions: &[(Modifiers, Key, UiAction)]) -> Vec<(egui::KeyboardShortcut, UiAction)> {
        actions
            .iter()
            .map(|(m, k, a)| (egui::KeyboardShortcut::new(*m, *k), a.clone()))
            .collect()
    }
...
pub struct TrivialConfig {
    ...
    /// Maximum height of the table body scroll area. If `None`, no limit (table can grow).
