...
pub(crate) struct Clipboard<R> {
    pub(crate) slab: Box<[R]>,

    /// The first tuple element `VisRowPos` is offset from the top-left corner of the
    /// selection.
    pub(crate) pastes: Box<[(VisRowOffset, ColumnIdx, RowSlabIndex)]>,
}
...
impl<R> UiState<R> {
    ...
    /// Take the clipboard contents (e.g. when preserving across table replace).
    pub fn take_clipboard(&mut self) -> Option<Clipboard<R>> {
        self.clipboard.take()
    }

    /// Restore clipboard contents (e.g. after table replace).
    pub fn set_clipboard(&mut self, c: Option<Clipboard<R>>) {
        self.clipboard = c;
    }
    ...
    pub fn try_update_clipboard_from_string<V: RowViewer<R>>(
        ...
        */
        ...
        // Use selection for paste target, or interactive cell when no selection
        let selection_offset = if let CursorState::Select(sel) = &self.cc_cursor {
            sel.first().map_or_else(
                || {
                    let (_, c) = self.cc_interactive_cell.row_col(self.p.vis_cols.len());
                    c.0
                },
                |idx| {
        ...
        let view = csv::parse_clipboard(contents);
        ...
        for row_offset in 0..view.num_rows() {
            ...
            for column in 0..view.num_columns_at(row_offset) {
                let data = view.get_cell(row_offset, column).unwrap_or("");
    ...
    fn try_dump_clipboard_content<V: RowViewer<R>>(
        ...
        for (row, columns, ..) in &clipboard.pastes.iter().chunk_by(|(row, ..)| *row) {
            while row_cursor < row.0 {
                csv::write_csv_newline(&mut buf_out);
                row_cursor += 1;
            }
            ...
            for (_, column, data_idx) in columns.into_iter() {
                while column_cursor < column.0 - column_offset {
                    csv::write_csv_comma(&mut buf_out);
                    column_cursor += 1;
                }
                ...
                csv::write_csv_field(&mut buf_out, &buf_tmp);
                buf_tmp.clear();
            }
        }
    ...
    /// Returns column indices in display order (for export, etc.).
    pub fn column_order(&self) -> Vec<usize> {
        self.p.vis_cols.iter().map(|c| c.0).collect()
    }
    ...
    /// Returns row indices in display order (respecting current sort).
    /// Empty if no sort has been applied or cache is invalid.
    pub fn sorted_row_indices(&self) -> Vec<usize> {
        self.cc_rows.iter().map(|ri| ri.0).collect()
    }
    ...
    pub fn try_apply_ui_action(
        ...
        match action {
            ...
            UiAction::PasteFromSystemClipboardInPlace(contents) => {
                if self.try_update_clipboard_from_string(vwr, &contents) {
                    self.try_apply_ui_action(table, vwr, UiAction::PasteInPlace)
                } else {
                    vec![]
                }
            }
            UiAction::PasteFromSystemClipboardInsert(contents) => {
                if self.try_update_clipboard_from_string(vwr, &contents) {
                    self.try_apply_ui_action(table, vwr, UiAction::PasteInsert)
                } else {
                    vec![]
                }
            }
